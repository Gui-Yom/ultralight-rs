...
impl JSString {
    ...
    pub fn to_string(&self) -> String {
        unsafe {
            let max_size = ultralight_sys::JSStringGetMaximumUTF8CStringSize(self.raw);
            let mut buffer: Vec<u8> = Vec::with_capacity(max_size as usize);
            let actual_size = ultralight_sys::JSStringGetUTF8CString(
                self.raw,
                buffer.as_mut_ptr() as *mut ::std::os::raw::c_char,
                max_size,
            );
            buffer.set_len((actual_size - 1) as usize);
...
impl Drop for JSString {
    fn drop(&mut self) {
        unsafe { ultralight_sys::JSStringRelease(self.raw) }
    }
}

impl PartialEq for JSString {
    fn eq(&self, other: &JSString) -> bool {
        unsafe { ultralight_sys::JSStringIsEqual(self.raw, other.raw) }
    }
}
...
impl<'s> PartialEq<&'s str> for JSString {
    fn eq(&self, other: &&'s str) -> bool {
        ...
        unsafe { ultralight_sys::JSStringIsEqualToUTF8CString(self.raw, utf8.as_ptr()) }
    }
}
...
impl<'s> PartialEq<JSString> for &'s str {
    fn eq(&self, other: &JSString) -> bool {
        ...
        unsafe { ultralight_sys::JSStringIsEqualToUTF8CString(other.raw, utf8.as_ptr()) }
    }
}
...
impl<'s> From<&'s str> for JSString {
    fn from(s: &'s str) -> Self {
        ...
        JSString {
            raw: unsafe { ultralight_sys::JSStringCreateWithUTF8CString(c.as_ptr()) },
        }
    }
}

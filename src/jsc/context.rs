...
impl JSContext {
    ...
    pub fn new_with_class(global_object_class: &JSClass) -> Self {
        JSContext {
            raw: unsafe { ultralight_sys::JSGlobalContextCreate(global_object_class.raw) },
        }
    }

    /// Gets the context group to which a JavaScript execution context belongs.
    pub fn group(&self) -> JSContextGroup {
        let g = unsafe { ultralight_sys::JSContextGetGroup(self.raw) };
        unsafe {
            ultralight_sys::JSContextGroupRetain(g);
        };
    ...
    pub fn name(&self) -> Option<JSString> {
        let r = unsafe { ultralight_sys::JSGlobalContextCopyName(self.raw) };
        if r.is_null() {
            None
        } else {
            Some(JSString { raw: r })
    ...
    pub fn set_name<S: Into<JSString>>(&self, name: S) {
        unsafe { ultralight_sys::JSGlobalContextSetName(self.raw, name.into().raw) }
    }
}
...
impl Default for JSContext {
    ...
    fn default() -> Self {
        JSContext {
            raw: unsafe { ultralight_sys::JSGlobalContextCreate(ptr::null_mut()) },
        }
    }
}
...
impl Drop for JSContext {
    fn drop(&mut self) {
        unsafe { ultralight_sys::JSGlobalContextRelease(self.raw) }
    }
}
